...
Router.post('/deposit/:id', authMiddleware, async (req, res) => {
    ...
    await client.query(
        'insert into transactions(transaction_date, deposit_amount, account_id, remaining_balance) values($1,$2,$3,$4) returning *',
    ...
    const client = await getClient();
     try {
         ...
         if (withdraw_amount <= total_balance) {
             await client.query(
                 'insert into transactions(transaction_date, withdraw_amount, account_id, remaining_balance) values($1,$2,$3,$4) returning *',   
            ...
            await client.query('commit');
                ...
                return res.status(400).send({
                    withdraw_error: "Insufficient Funds: You don't have enough funds in your account."
                });
            }
            res.send();
        } catch (error) {
            await client.query('rollback');
                res.status(400).send({
                    withdraw_error: 'an error occured while withdrawing amount...Try again later.'
                });
        } finally {
            client.release();
        }
